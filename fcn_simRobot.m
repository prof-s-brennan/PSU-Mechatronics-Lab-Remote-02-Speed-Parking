function IR_distance = fcn_simRobot2(torque_L,torque_R,sensorx,sensory,sensor_angle_in_degrees,varargin)
...
%% Constrain the torques to +/- 100 percent
...
torque_friction_L = torque_coulomb_L * sign(robot.omega_L)*(1+randn*0.1)   + robot.omega_L*damping_L;
...
torque_friction_R = torque_coulomb_R * sign(robot.omega_R)*(1+randn*0.1)  + robot.omega_R*damping_R;
...
% Plot result
[IR_distance,front_distance] = fcn_drawRobot(robot,1);  % Redraw the robot in Figure 1
...
if abs(V)<=0.01
    robot.stop_count = robot.stop_count +1;

    if  robot.stop_count > 100  % Robot has to stay stationary for 1 second
        IR_distance = NaN;
        time_score = (robot.current_plot_index -100)*robot.delta_t;
        fprintf(1,'Your robot has come to rest for one second.\n');
        fprintf(1,'The simulation, in wall time, took: %.4f seconds to run.\n',(cputime-robot.start_time));
        fprintf(1,'Distance from front of the robot to wall was: %.4f cm.\n',front_distance);
        fprintf(1,'Simulated time from start to stop was: %.4f seconds.\n',time_score);
        fprintf(1,'Final score: %.4f\n',time_score + front_distance*10);
    end
else
    robot.stop_count = 0; % Reset the count if the robot starts moving again
end
...
function [IR_distance,front_distance] = fcn_drawRobot(robot,varargin)
...
if isempty(arena)
    ...
    %length_arena = 400;
    arena.wall_start = [-50 -50; 300 -50; 300 50; -50 50];
    ...
    arena.targets_start = [300 -50];
    arena.targets_end  = [300 50];
...
if flag_no_collision ==0
    IR_distance = NaN;
    fprintf(1,'Sadly, your robot ran into a wall and failed.\n');
    fprintf(1,'This occurred at a simulated time of: %.4f seconds.\n',(robot.current_plot_index*robot.delta_t));
    fprintf(1,'Wall time simulation duration was: %.4f seconds.\n', (cputime-robot.start_time));
end
...
%     fprintf(1,'Competition complete! Your simulated completion time is: %.4f seconds. Wall time simulation duration was: %.4f seconds.\n',(robot.current_plot_index*robot.delta_t), (cputime-robot.start_time));
